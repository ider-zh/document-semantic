...
P _ {M} (x) = \sum_ {i = 0} ^ {\infty} 2 ^ {- | S _ {i} (x) |}

# 2. 历史：柯尔莫哥罗夫与复杂性

所罗门诺夫归纳法可以如下定义：给定序列( \mathsf { x } _ { 1 } , \mathsf { x } _ { 2 } , \ldots , \mathsf { x } _ { \mathsf { n } } )）, 预测x _ { n + 1 0 }。归纳推理就是力图找到一个最小的图灵机，可以为( \mathsf { x } _ { 1 } , \mathsf { x } _ { 2 } , \ldots , \mathsf { x } _ { \mathsf { n } } )）建模，从而准确地预测后续序列。序列的描述长度就是图灵机的大小，这其实就是麦卡锡最初模糊地意识到的“有效”。例如，如果一个序列是\mathsf { n }个1: （1, 1, 1,…）,那么我们可以写出如下程序输出该序列:

```
for   \mathbf{i} = 1  to n print 1
```
...
![所罗门诺夫与蔡廷，2003](resources/images/image_2.jpg)
...
\operatorname {K t} (\mathrm {x}) = \min  \left\{\ell (\mathrm {p}) + \log (\text {t i m e} (\mathrm {p})) \colon \mathrm {U} (\mathrm {p}) = \mathrm {x} \right\}

这样列文的复杂性就是可计算的了，也就是说，如果逆函数存在，总可以通过列文的通用搜索过程找到。这和所罗门诺夫更早获得的收敛性定理契合。

![](resources/images/image_3.jpg)
