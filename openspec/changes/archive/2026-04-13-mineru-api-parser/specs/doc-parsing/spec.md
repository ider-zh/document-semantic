...
The system SHALL provide a `ParserRegistry` that maps string identifiers (e.g., `"pandoc"`, `"python-docx"`, `"markdownit"`, `"mineru"`) to parser classes. The pipeline SHALL resolve the active parser from configuration at runtime.
...
#### Scenario: Registry lookup mineru
