...
The system SHALL include end-to-end tests that run a complete DOCX → `SemanticDocument` pipeline with all components wired together. Integration tests SHALL verify: correct block ordering, inline element extraction, attachment references, and JSON serialization round-trip. Integration tests SHALL additionally support real DOCX files from `tests/docx/` via the test routing system. When running against a real DOCX file, the test SHALL use the processor configuration and assertions defined in the test route for that file.
