...
class LLMRecognizer(SemanticRecognizer):
    ...
    def recognize(self, intermediate: IntermediateResult) -> SemanticDocument:
        ...
        raise NotImplementedError(
            "LLMRecognizer is not yet fully implemented. This is a placeholder for future LLM integration."
        )
    ...
    def recognize(self, intermediate: IntermediateResult) -> SemanticDocument:
        ...
        for rule in self._rules:
            if rule.matches(metadata):
                logger.info(f"[recognition:router] Matched rule -> {rule.recognizer.name}")
...
def create_recognizer(
    ...
    if cls is None:
        raise RecognizerNotFoundError(recognizer_type, list(_RECOGNIZER_REGISTRY.keys()))
    return cls(**config)
