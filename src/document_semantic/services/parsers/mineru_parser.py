...
def _get_mineru_token(config: Settings | None = None) -> str | None:
    """Get the MinerU API token from config or environment."""
    if config and config.mineru_token:
        return config.mineru_token
    return os.getenv("MINERU_TOKEN") or os.getenv("token_mineru")


def _get_cache_dir(config: Settings | None = None) -> Path:
    """Get the cache directory for MinerU results."""
    if config and config.mineru_cache_dir:
        return Path(config.mineru_cache_dir)
...
def _get_pdf_cache_dir(config: Settings | None = None) -> Path:
    """Get the cache directory for MinerU PDF results."""
    if config and config.mineru_pdf_cache_dir:
        return Path(config.mineru_pdf_cache_dir)
    pdf_cache_dir = os.getenv("MINERU_PDF_CACHE_DIR")
    if pdf_cache_dir:
        return Path(pdf_cache_dir)
    # Default is a 'pdf' subdirectory within the main mineru cache
    return _get_cache_dir(config) / "pdf"
...
def _check_cache(
    docx_path: Path, skip_image_ocr: bool = False, config: Settings | None = None
    ...
    cache_dir = _get_cache_dir(config) / file_hash
...
def _save_to_cache(
    ...
    placeholder_zip_data: bytes | None = None,
...
def _generate_placeholder_image() -> bytes:
    ...
    try:
        ...
        img = Image.new(
            "RGB",
            (_PLACEHOLDER_WIDTH, _PLACEHOLDER_HEIGHT),
            (_PLACEHOLDER_GRAYAY, _PLACEHOLDER_GRAYAY, _PLACEHOLDER_GRAYAY),
        )
        buf = io.BytesIO()
...
def _replace_images_in_docx(
    ...
    with zf.ZipFile(docx_path, "r") as src_zip, zf.ZipFile(output_path, "w", zf.ZIP_DEFLATED) as dest_zip:
...
class MinerUParser(Parser):
    ...
    def parse(
        ...
        token = _get_mineru_token(config)
        if not token:
        ...
        # Check cache first
        cached = _check_cache(docx_path, skip_image_ocr, config)
        ...
        zip_data, original_images_dir, modified_docx_path = self._upload_and_download(docx_path, token, skip_image_ocr)
        ...
        try:
            ...
            _save_to_cache(docx_path, result, zip_data, skip_image_ocr, placeholder_zip_data, config)
        finally:
    ...
    def process(
        ...
        skip_image_ocr: bool = False,
        pipeline_config: Settings | None = None,
    ) -> ProcessResult:
        ...
        zip_data = self._get_zip_data(docx_path, skip_image_ocr, pipeline_config)
    ...
    def _get_zip_data(self, docx_path: Path, skip_image_ocr: bool = False, config: Settings | None = None) -> bytes:
        ...
        if cached is not None:
            ...
            cache_dir = _get_cache_dir(config) / _compute_file_hash(docx_path)
            zip_name = "result_restored.zip" if skip_image_ocr else "result.zip"
            zip_path = cache_dir / zip_name
            if zip_path.exists():
    ...
    def _upload_and_download(
        ...
        token: str,
        skip_image_ocr: bool = False,
    ) -> tuple[bytes, Path | None, Path | None]:
        ...
        """
        modified_docx_path: Path | None = None
        original_images_dir: Path | None = None
        ...
        with httpx.Client(timeout=HTTP_TIMEOUT_SECONDS) as client:
            ...
            if resp.status_code != 200:
                raise ParserError(f"Failed to get upload URL: HTTP {resp.status_code}: {resp.text}")
            ...
            if not batch_id or not urls:
                raise ParserError(f"Unexpected API response structure: {result}")
            ...
            if upload_resp.status_code != 200:
                raise ParserError(f"Upload failed: HTTP {upload_resp.status_code}: {upload_resp.text}")
            ...
            logger.info("[parsing:mineru] Downloading result ZIP")
            zip_resp = client.get(zip_url, timeout=HTTP_TIMEOUT_SECONDS)
            if zip_resp.status_code != 200:
                raise ParserError(f"Download failed: HTTP {zip_resp.status_code}")

            return zip_resp.content, original_images_dir, modified_docx_path

    def _poll_for_result(self, client: httpx.Client, headers: dict[str, str], batch_id: str) -> str:
        ...
        while True:
            ...
            if elapsed > POLL_TIMEOUT_SECONDS:
                raise ParserError(f"Polling timeout after {elapsed:.0f}s for batch {batch_id}")

            resp = client.get(url, headers=headers)
            if resp.status_code != 200:
                raise ParserError(f"Poll failed: HTTP {resp.status_code}: {resp.text}")
            ...
            if state == "done":
                ...
                logger.info("[parsing:mineru] Extraction complete")
                return zip_url
            elif state == "failed":
    ...
    def _parse_zip(self, zip_data: bytes, docx_path: Path) -> IntermediateResult:
        ...
        if "content_list_v2.json" in file_names:
            ...
            try:
                from document_semantic.services.parsers.mineru_aligner import DocxAligner

                aligner = DocxAligner(str(docx_path))
                flat_items = aligner.align(flat_items)
            ...
            blocks, attachments = self._parse_content_list(flat_items, zf)
        else:
        ...
        logger.info(f"[parsing:mineru] Extracted {len(blocks)} blocks, {len(attachments)} attachments from result ZIP")
    ...
    def _item_to_block(self, item: dict[str, Any], attachments: list[Attachment]) -> IntermediateBlock | None:
    ...
    def _extract_attachments_from_zip(self, zf: zipfile.ZipFile, file_names: list[str]) -> list[Attachment]:
...
class _MinerUContentConverter:
    ...
    def convert(
        ...
        try:
            ...
            aligner = DocxAligner(self._source_path)
            items = aligner.align(items)
        except Exception as e:
            logger.error(f"[parsing:mineru] Failed to align docx text: {e}", exc_info=True)
    ...
    def _list_to_markdown(self, content: dict, elem_type: str, use_placeholders: bool) -> list[str]:
