...
def assert_block_types(doc: SemanticDocument, expected_types: list[str]) -> None:
...
def assert_block_content_contains(doc: SemanticDocument, index: int, substring: str) -> None:
    ...
    if index < 0 or index >= len(doc.blocks):
        raise AssertionError(f"Block index {index} out of range (document has {len(doc.blocks)} blocks)")
    content = doc.blocks[index].content
    if substring not in content:
        raise AssertionError(f"Block {index} does not contain '{substring}'. Actual content: {content!r}")
...
def assert_inline_element_count(doc: SemanticDocument, block_index: int, element_type: str, expected: int) -> None:
    ...
    if block_index < 0 or block_index >= len(doc.blocks):
        raise AssertionError(f"Block index {block_index} out of range (document has {len(doc.blocks)} blocks)")
    block = doc.blocks[block_index]
    actual_count = sum(1 for el in block.inline_elements if el.type == element_type)
    if actual_count != expected:
        raise AssertionError(
            f"Block {block_index}: expected {expected} '{element_type}' inline elements, got {actual_count}"
        )
...
def assert_inline_element_text(
    ...
    if block_index < 0 or block_index >= len(doc.blocks):
        raise AssertionError(f"Block index {block_index} out of range (document has {len(doc.blocks)} blocks)")
    block = doc.blocks[block_index]
    typed_elements = [el for el in block.inline_elements if el.type == element_type]
...
def assert_document_partial(actual: SemanticDocument, expected: dict[str, Any]) -> list[str]:
    ...
    if "blocks" in expected:
        for idx_str, block_expect in expected["blocks"].items():
            ...
            if block is None:
                failures.append(f"Block {block_idx}: does not exist (document has {len(actual.blocks)} blocks)")
                continue

            # Type check
            if "type" in block_expect and block.type != block_expect["type"]:
                failures.append(f"Block {block_idx}: expected type '{block_expect['type']}', got '{block.type}'")
            ...
            if "content_contains" in block_expect:
                for substring in block_expect["content_contains"]:
                    if substring not in block.content:
                        failures.append(
                            f"Block {block_idx}: does not contain '{substring}'. Content: {block.content!r}"
                        )
            ...
            if "inline_elements" in block_expect:
                for el_expect in block_expect["inline_elements"]:
                    ...
                    typed = [el for el in block.inline_elements if el.type == el_type]
                    if el_idx < len(typed):
