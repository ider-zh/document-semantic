...
from .protocol import SemanticRecognizer, RecognizerNotFoundError
...
def _classify_block(block: IntermediateBlock) -> Block:
    ...
    if heading_match:
        ...
        return HeadingBlock(
            ...
            inline_elements=_extract_or_merge_inline_elements(
                heading_match.group(2).strip(), pre_existing_inlines
            ),
        )
...
def _extract_or_merge_inline_elements(
    text: str, pre_existing: list[InlineElement]
) -> list[InlineElement]:
    ...
    def recognize(self, intermediate: IntermediateResult) -> SemanticDocument:
        ...
        for i, raw_block in enumerate(intermediate.blocks):
            try:
                ...
                logger.warning(
                    f"[recognition:regex] Failed to classify block {i}: "
                    f"content_preview='{raw_block.content[:50]}...'"
                )
                ...
                blocks.append(
                    TextBlock(
                        ...
                        inline_elements=_extract_or_merge_inline_elements(
                            raw_block.content, raw_block.inline_elements
                        ),
                    )
                )
        ...
        attachments = [
            Attachment(id=a.id, path=a.path, mime_type=a.mime_type)
            for a in intermediate.attachments
        ]
        ...
        logger.info(
            f"[recognition:regex] Complete: {len(blocks)} blocks classified"
        )
        return doc
