...
class IntermediateResult(BaseModel):
    ...
    blocks: list[IntermediateBlock] = Field(
        default_factory=list, description="Raw blocks extracted from the document"
    )
...
class ParserNotFoundError(ParserError):
    ...
    def __init__(self, parser_name: str, available_parsers: list[str]):
        ...
        super().__init__(
            f"Parser '{parser_name}' not found. Available parsers: {', '.join(available_parsers)}"
        )
