...
class MarkdownitParser(Parser):
    ...
    def parse(self, docx_path: Path, skip_image_ocr: bool = False) -> IntermediateResult:
        ...
        if not _markdown_it_available():
            raise ParserDependencyError(
                ...
                message=(
                    "markdown-it-py is not installed. Install with: "
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            logger.info(
                f"[parsing:markdownit] Complete: {len(blocks)} blocks, "
    ...
    def _create_image_attachment(
        self, token, media_dir: Path, index: int
    ) -> Attachment | None:
